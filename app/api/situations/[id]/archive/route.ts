...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  const updated = await prisma.situation.update({ where:{id:params.id}, data:{status:"ARCHIVED"} });
  await prisma.auditEvent.create({data:{action:"SITUATION_ARCHIVED",entityType:"Situation",entityId:params.id}});
